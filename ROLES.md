# AquaSense — User Roles & Permissions Matrix

This document outlines the architecture, features, and role-based access control (RBAC) rules for **AquaSense: Smart Water Quality Monitoring and Management System** at Camarines Sur Polytechnic Colleges (CSPC).

---

## 1. Project Overview
AquaSense is an IoT-powered real-time water quality monitoring system designed for campus-wide safety. ESP32 microcontrollers measure physical and chemical water parameters and publish them via MQTT to a central Node.js backend. The system automatically classifies water safety using PNSDW (Philippine National Standards for Drinking Water) and WHO guidelines.
...
* **IoT Protocol:** MQTT subscription on HiveMQ public broker (`esp32/aquasense/#`).
...
The **Admin** has full read and write administrative privileges across the entire AquaSense platform. They are responsible for keeping the system running, enrolling users, configuring parameters, and managing physical hardware devices.
